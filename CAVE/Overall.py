...
from lowrank import OLELoss,OLELossnum,OLELoss_cuda,OLELoss_cuda_MODE,OLELoss_cpu, OLELoss_cuda_ralative
port = 8100
...
#     B,C,H,W = xin.shape
#     i = 32
#     xin = torch.reshape(xin,[B,C,H//i,i,W//i,i])
#     gt = torch.reshape(gt,[B,C,H//i,i,W//i,i])
#     xin = xin.permute([2,4,0,1,3,5])
#     gt = gt.permute([2,4,0,1,3,5])
#     # H/8 , W/8 , B, C, 8, 8
#     xin = torch.reshape(xin,[H//i*W//i*B,C,i*i])
#     gt = torch.reshape(gt,[H//i*W//i*B,C,i*i])
#     [u1,s1,v1] = torch.svd(gt)
#     [u2,s2,v2] = torch.svd(xin)
#     mask1 = s1>1e-3
#     mask2 = s2>1e-3
#     mask1 = mask1.float()
#     mask2 = mask2.float()
#     s1 = s1*mask1*mask2
#     s2 = s2*mask1*mask2
#     delta = torch.abs(torch.sum(s1,1)-torch.sum(s2,1))
...
if __name__ == '__main__':
    ...
    parse.add_argument('--batch_size',type = int, default = 10, help = 'size of the batches')
    ...
    for epoch in range(opt.n_epochs):
        ...
        for hsi,hsi_g,hsi_resize,msi in Hyper_train:
            ...
            # rank_loss = low_rank(refined.to(device),hsi_g.to(device))
            # else:
            # refined.register_hook(save_grad('refined'))
            rank_loss = OLELoss_cuda_ralative.apply(refined.to(device),hsi_g.to(device)) 
            loss = MaeLoss(fout.to(device),hsi_g.to(device))+MaeLoss(refined.to(device),hsi_g.to(device))+rank_loss
            # rank_loss.backward()
            # del hsi_g
            loss.backward()
            # test = grads['refined']
            # print('grad:{}'.format(test))
            ...
            del loss
            if opt.local_rank == 0:
                print('[Epoch:{}/{}][batch:{}/{}][loss:{}][learning rate:{}][low_rank:{}][time using:{}]'.format(epoch,opt.n_epochs,batch,b_r,loss_[-1],optimzier.state_dict()['param_groups'][0]['lr'], rank_loss.item(),time.time() - localtime))
            localtime = time.time()
